...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  Future<void> _editConcerns() async {
    const all = [
      ...
      ('redness', 'Покраснения'),
      ('rosacea', 'Розацеа / купероз'),
      ('sensitivity', 'Чувствительная кожа'),
      ('aging', 'Признаки старения'),
      ('wrinkles', 'Морщины'),
      ('elasticity', 'Упругость и плотность'),
      ('dullness', 'Тусклый тон'),
      ('pigmentation', 'Пигментация'),
      ('texture', 'Неровный рельеф'),
      ('dark_circles', 'Круги под глазами'),
      ('puffiness', 'Отёчность'),
      ('barrier', 'Восстановление барьера'),
      ('post_procedure', 'После процедур'),
    ];
